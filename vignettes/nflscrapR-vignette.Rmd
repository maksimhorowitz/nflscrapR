...
author: "Maksim Horowitz (@bklynmaks)"
date: "`r Sys.Date()`"
...
Use the following code.  The devtools packages is needed to download `nflscrapR` as the package is currently hosted on Github:
...
The `nflscrapR` package contains two `play-by-play` functions.  The single game function outputs a 62 column dataframe and the season long function outputs a 63 column dataframe, each with detailed information about each play including pre-snap informations, play call, and post snap results.  To explore the documentation of the function and to learn more about what each column describes, use the following code:
...
*Note: There are errors within the NFL API raw data. Numerous extra-point scores are 
missing due to what seems to be a bug in the API.  Alas, this does not have a major
effect on the games outside of a few point difference in scores.*
...
As seen above the Ravens ran more plays than the 49ers.  There are many directions
one can move for further insight.  Some ideas are as follows:
...
- Take a further step and look at plays by drive to see if there was one 
specific drive that created the differences in total offensive plays between
the teams
...
with 56 columns while the game level function outputs a dataframe with 55 columns.
...
- `season_player_game`: outputs a dataframe for an entire season.  **One line per player per game**.  All statistics measured statistics are reported.
...
# Loading all the statistics for each player by game from 2009-2015
# Note: The below code takes about 15 minutes to run.
...
# Combining into one dataframe using rbind()
...
# filter for Flacco
flacco_data <- filter(allplayerstats, name == "J.Flacco")
flacco_data <- flacco_data[order(flacco_data$date),]

# Add games player.  Note in 2015 Flacco was injured in Week 10
flacco_data$gamenumber <- as.factor(c(rep(1:16, times = 6), 1:10))
...
The plot below of Flacco's passing yards across games provides an interesting visual.
By examining the plot one can see that he usually has a strong first game followed by a much weaker second game (excluding the 2015 season).  One can also see that in 2015 Flacco was one pace to throw for a career high in yards in a season before he was derailed by a torn ACL.  

```{r, echo = FALSE, fig.align= "center", fig.height= 9, fig.width=9}
flacco_passyds_plot
```

Now examined are Flacco's pass attempts across games.  This allows visualization of 
his progression from a game manager to the elite level quarterback that he his today. What one can observe the following insights from the bar plots below:

- Flacco's most consistent season in terms of pass attempts was 2010
- In 2013 and 2014 the Ravens started the season aggressively relying heavily on
the passing game with 62 attempts in the first game
- In 2009, 2010, and 2011 Flacco's attempts tailed off at the end of the seasons. This could be related to the Ravens having already clinched a playoff berth by that point in the season.
...
function.  The nice thing about the `player_game` function is that it downloads, parses, and cleans the data very quickly so if you are interested in particular games, you can download the data almost instantaneously.

The `agg_player_season` function allows users to visualize season total statistics which is beneficial for building running list of statistics or easily calculating total from the available range of seasons. 

Note, there is also a simple box score function available which separates each of the
measured statistics into different lists of dataframes.  This is much more similar what is seen in a standard box score.  The function is named: `simple_boxscore`
...
*Note: The `season_rosters` function takes a few minutes to run*
...
The `drive_summary` function allows users to download data sets with information about each drive in a game.  The input requires a GameID, and the outputted dataframe has
18 columns.  The help documentation for the function describes each of the columns 
in more detail.  Drives summaries provide interesting insight into what type of play calling leads to specific type of drive results such as scoring plays, punts, or turnovers.

### Using the drive_summary function

Displayed below is an interactive table of the drive summaries of the last game of the 2015 season between the Minnesota Viking and the Green Bay Packers.
```{r}
